...
fun expressionToUnicodeString(
        expressionNode: ExpressionNode,
        characterEscapingDepth: Int = 1
) = escapeCharacters(expressionNode.toPlainTextView(functionNameGetter = { it.unicodeRepresentation }), characterEscapingDepth)
...
fun structureStringToTexString(
        structureString: String,
        scope: String = "",
        functionConfiguration: FunctionConfiguration = FunctionConfiguration(
                scopeFilter = scope.split(";").filter { it.isNotEmpty() }.toSet()
