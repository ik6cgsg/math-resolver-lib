...
data class ExpressionNode(
        ...
        var lastAppliedSubstitution: ExpressionSubstitution? = null,
        var lastModifiedStepId: Int = -1
) {
    ...
    fun setVariable(value: Double) {
        val stringValue = value.toShortString()
    ...
    fun setLastAppliedSubstitutionForAllNodes(substitution: ExpressionSubstitution) {
        this.lastAppliedSubstitution = substitution
        for (child in children) {
            child.setLastAppliedSubstitutionForAllNodes(substitution)
        }
    }

    fun setStepIdForAllNodes(stepId: Int) {
        this.lastModifiedStepId = stepId
        for (child in children) {
            child.setStepIdForAllNodes(stepId)
        }
    }
    ...
    fun isNumberValue() = value.isNotBlank() && (value.first().isNumberPart() || value == "π" || value == "e") //TODO: 'value == "e"' leads that 'e' can not be variable

    fun isIntNumber(): Boolean {
        if (value.isEmpty() && children.size == 1) {
            return children.first().isIntNumber()
        }
        if (value.isBlank() || !value.first().isNumberPart()) {
            return false
        }

        val number = value.toDoubleOrNull() ?: return false
        val numberFloor = floor(number)
        if (!BaseOperationsComputation.additivelyEqual(numberFloor, number)) {
            return false
        }
    ...
    fun isOddNumber(): Boolean {
        if (value.isEmpty() && children.size == 1) {
            return children.first().isOddNumber()
        }
        if (value.isBlank() || !value.first().isNumberPart()) {
            return false
        }
        ...
        val number = value.toDoubleOrNull() ?: return false
        val numberDiv2 = number.toInt() / 2
        if (!BaseOperationsComputation.additivelyEqual(numberDiv2 * 2.0 + 1.0, number)) {
            return false
        }
    ...
    fun getNodeValueString() = when (value){
        PI_STRING -> PI_STRING_USUAL
        E_STRING -> E_STRING_USUAL
        else ->value
    }

    fun getComputeNodeValue(computeExpressionVariableType: ComputeExpressionVariableType) = when (computeExpressionVariableType) {
        ComputeExpressionVariableType.ALL_TO_NUMBERS -> value
        ComputeExpressionVariableType.ALL_TO_NUMBERS_OR_PI_OR_E,
        ComputeExpressionVariableType.ALL_TO_NUMBERS_OR_PI_OR_E_ADDITIVELY_GROUPED -> getNodeValueString()
    }

    fun getTexNodeValueString() = when (value){
        PI_STRING -> PI_STRING_TEX
        else ->value
    }
    ...
    fun getFirstNotBracketChild(): ExpressionNode {
        var result = this
        while (result.value == "" && result.children.size == 1) {
    ...
    fun normalizeNumbers() {
        if (nodeType == NodeType.VARIABLE) {
            ...
            children.forEach { it.normalizeNumbers() }
        }
    }

    fun normalizeTrivialFunctions() {
        children.forEach { it.normalizeTrivialFunctions() }
        if (nodeType == NodeType.FUNCTION && functionStringDefinition?.function?.numberOfArguments == -1 &&
                functionStringDefinition?.function?.notObligateMainFunction() == functionStringDefinition?.function?.function &&
                children.size == 1 && children.first().functionStringDefinition?.function?.mainFunction != value ) {
            setNode(children.first())
        }
    }

    fun normalizeSubtructions(functionConfiguration: FunctionConfiguration) { //TODO: generalize because it depends on configuration
        for (i in children.lastIndex downTo 0) {
            children[i].normalizeSubtructions(functionConfiguration)
            if (children[i].nodeType == NodeType.VARIABLE && children[i].value.startsWith("-")) {
                val double = children[i].value.toDoubleOrNull()
                if (double != null && double > 0 && double.toInt().toDouble() == double) {
                    setVariable(double)
    ...
    fun normalizeNullWeightCommutativeFunctions() {
        for (i in children.lastIndex downTo 0) {
            ...
            if (children[i].children.size == 1 && (children[i].functionStringDefinition?.function?.isCommutativeWithNullWeight == true || children[i].value == "")) {
    ...
    fun toPlainTextView(
            ...
            getNodeValueString: (ExpressionNode) -> String = { it.getNodeValueString() },
            functionNameGetter: (FunctionProperties) -> String = { it.plainTextRepresentation }
    ): String {
        var result = toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)
    ...
    private fun toPlainTextViewRec(
            ...
            getNodeValueString: (ExpressionNode) -> String,
            functionNameGetter: (FunctionProperties) -> String
    ): String {
        var identifier = ""
        if (nodeType == NodeType.VARIABLE) {
            identifier = getNodeValueString(this)
        } else if (value == "" && children.size == 1) {
            identifier = children.first().toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)
            ...
            when (functionIdentifier.defaultStringDefinitionType) {
                StringDefinitionType.BINARY_OPERATION -> {
                    ...
                    if (children.size > 0) {
                        ...
                        for (i in 1..children.lastIndex) {
                            if (functionConfiguration.functionPropertiesByName.get(children[i].value + "_-1")?.mainFunction != functionIdentifier.function ||
                                    functionConfiguration.functionPropertiesByName.get(children[i].value + "_-1")?.function == functionIdentifier.function) {
                                identifier += functionNameGetter(functionIdentifier)
                            }
                            identifier += children[i].toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)
                        }
                    }
                    if (binaryOperationNeedBrackets()) {
                        identifier += ")"
                ...
                StringDefinitionType.UNARY_LEFT_OPERATION -> {
                    if (functionConfiguration.functionProperties.find { it.function == parent?.value }?.defaultStringDefinitionType in listOf(StringDefinitionType.FUNCTION, StringDefinitionType.BINARY_OPERATION)) {
                        identifier += functionNameGetter(functionIdentifier) + children.first().toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)
                    } else {
                        identifier += "(" + functionNameGetter(functionIdentifier) + children.first().toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter) + ")"
                    }
                }
                StringDefinitionType.UNARY_RIGHT_OPERATION -> {
                    val childplainTextRepresentation = children.first().toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)
                    identifier = if (childplainTextRepresentation.first().isSign(' ',false)) {
                        "($childplainTextRepresentation)"
                    } else {
                        childplainTextRepresentation
                    } + functionNameGetter(functionIdentifier)
                }
            }
        } else {
            identifier = value + "("
            for (child in children) {
                identifier += /*if (child.identifier.isEmpty()) */child.toPlainTextViewRec(functionConfiguration, getNodeValueString, functionNameGetter)/* else child.identifier*/ + ","
            }
            if (children.size > 0)
                identifier = identifier.substring(0, identifier.length - 1)
            identifier += ")"
    ...
    fun toTexView(
            functionConfiguration: FunctionConfiguration = FunctionConfiguration(),
            getNodeValueString: (ExpressionNode) -> String = { it.getTexNodeValueString() }
    ): String {
        ...
        if (nodeType == NodeType.VARIABLE) {
            ...
            if (functionIdentifier != null && functionIdentifier.texStringDefinitionType != StringDefinitionType.FUNCTION) {
                when (functionIdentifier.texStringDefinitionType) {
                    StringDefinitionType.BINARY_OPERATION -> {
                        if (binaryOperationNeedBrackets(isTex = true)) {
                            identifier = texOpenBracket
                        }
                        if (value == "/" && children.size == 2) {
                            identifier += "\\frac{" + children.first().getFirstNotBracketChild().toTexViewRec(functionConfiguration) + "}{" +
                                    children.last().getFirstNotBracketChild().toTexViewRec(functionConfiguration) + "}"
                            ...
                            for (i in 1..children.lastIndex) {
                                ...
                                identifier += children[i].toTexViewRec(functionConfiguration)
                            }
                        }
                        if (binaryOperationNeedBrackets(isTex = true)) {
                    ...
                    StringDefinitionType.UNARY_LEFT_OPERATION -> {
                        identifier += " ${functionIdentifier.texRepresentation} " + children.first().toTexViewRec(functionConfiguration)
                    }
                    StringDefinitionType.UNARY_RIGHT_OPERATION -> {
    ...
    private fun binaryOperationNeedBrackets(bracketThresholdAdditive: Double = 0.2, isTex: Boolean = false) =
            parent?.functionStringDefinition?.function?.defaultStringDefinitionType != StringDefinitionType.FUNCTION && (
                    ...
                    (parentIsNotFraction() || !isTex)

    private fun parentIsNotFraction() = (parent?.value != "/" || parent?.children?.size != 2)
    ...
    fun isComputableConstant(): Boolean {
        return isConstant() && children.size > 0
    }
    ...
    fun containsVariables(names: Set<String> = emptySet()): Boolean {
        if (nodeType == NodeType.VARIABLE) {
            if (value.toDoubleOrNull() == null && (names.isEmpty() || names.contains(value)))
                return true
        } else for (child in children) {
            if (child.containsVariables(names))
                return true
        }
        return false
    }
    ...
    fun getVariableNames(withoutNames: Set<String> = emptySet()): Set<String> {
        var result = mutableSetOf<String>()
        if (nodeType == NodeType.VARIABLE) {
            if (value.toDoubleOrNull() == null && value != "sys_def_i_complex" && !withoutNames.contains(value)) {
                result.add(value)
            }
        } else {
            val extraNames = if ((functionStringDefinition?.function?.numberOfDefinitionArguments ?: 0) > 0) {
                enrichNodeDefinitionArgumentsNames(withoutNames)
            } else withoutNames
            for (i in (functionStringDefinition?.function?.numberOfDefinitionArguments ?: 0)..children.lastIndex) {
                result.addAll(children[i].getVariableNames(extraNames))
            }
        }
        return result
    }

    fun countNumberOfVariablesInMap(counter: MutableMap<String, Int>) {
        if (nodeType == NodeType.VARIABLE) {
        ...
        for (child in children) {
            child.countNumberOfVariablesInMap(counter)
        }
    }

    fun getNonZeroVariables(withoutNames: Set<String> = emptySet()): Set<String> {
        ...
        for (i in (functionStringDefinition?.function?.numberOfDefinitionArguments ?: 0)..children.lastIndex) {
            result.addAll(children[i].getNonZeroVariables(extraNames))
        }
        return result
    }

    private fun enrichNodeDefinitionArgumentsNames(withoutNames: Set<String>): MutableSet<String> {
        val newWithoutNames = mutableSetOf<String>()
        for (i in 0 until (functionStringDefinition?.function?.numberOfDefinitionArguments ?: 0)) {
            newWithoutNames.add(children[i].value)
        }
        newWithoutNames.addAll(withoutNames)
    ...
    fun cloneAndWrap(): ExpressionNode {
        val result = ExpressionNode(NodeType.FUNCTION, "")
    ...
    fun replaceNotDefinedFunctionsOnVariables(
        ...
        if (nodeType == NodeType.FUNCTION && value.isNotBlank() && !definedFunctionNameNumberOfArgsSet.contains(value + "_" + children.size)
                && !definedFunctionNameNumberOfArgsSet.contains(value + "_-1")
        ) {
...
class ExpressionNodeConstructor(
    ...
    fun construct(identifier: String): ExpressionNode {
        ...
        val child = if (identifier[0] == '(' && identifier.last() == ')') {
...
fun normalizeExpressionsForComparison(
    ...
    else if (right.nodeType == NodeType.FUNCTION && right.value == "") Pair(left.wrap(), right)
...
val zero = ExpressionNode(NodeType.VARIABLE, "0").wrap()
val one = ExpressionNode(NodeType.VARIABLE, "1").wrap()