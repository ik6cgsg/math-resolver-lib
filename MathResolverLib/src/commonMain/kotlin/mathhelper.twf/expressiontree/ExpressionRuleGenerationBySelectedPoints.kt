...
import mathhelper.twf.standartlibextensions.containsAny
import mathhelper.twf.standartlibextensions.isDigit
...
enum class ForwardInverseExtensionType {
    ADD_SUBTRACT,
    MULTIPLY_DIVIDE,
    POW_ROOT,
    EXPONENTIATE_LOGARITHM,
    LOGIC_ABSORPTION
}

data class ForwardInverseExtension(
        val forwardInverseExtensionType: ForwardInverseExtensionType,
...
fun generateReduceFractionSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey(REDUCE_FRACTION) &&
        ...
        if (substitutionSelectionData.selectedSubtreeTopArguments != null && substitutionSelectionData.notSelectedSubtreeTopArguments != null &&
                substitutionSelectionData.selectedSubtreeTopArguments!!.value == "/" && substitutionSelectionData.selectedNodeIds.size == 2 &&
                substitutionSelectionData.selectedSubtreeTopArguments!!.children.all { it.children.size == 1 } &&
                !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.fastProbabilityCheckOnZero(substitutionSelectionData.selectedSubtreeTopArguments!!.children.last())) {
            val originalExpression = substitutionSelectionData.originalExpression.clone()
            ...
            if (numerator.expressionStrictureIdentifier == denominator.expressionStrictureIdentifier) {
                ...
                val resNode: ExpressionNode? = if (substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.baseOperationsDefinitions.additivelyEqual(num, denom * numDivDenom)) {
                    ExpressionNode(NodeType.VARIABLE, numDivDenom.toString())
                } else if (substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.baseOperationsDefinitions.additivelyEqual(denom, num * denomDivNum)) {
                    substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("/", -1).apply {
                        addChild(ExpressionNode(NodeType.VARIABLE, "1"))
                ...
                if (numPow != null && denomPow != null) {
                    val resNode = if (numPow >= denomPow) {
                        ...
                        if (substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.baseOperationsDefinitions.additivelyEqual(pow, 1.0)) {
...
fun generateComplicatingExtensionSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.selectedNodes.size == 2) {
        ...
        if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("AdditiveComplicatingExtension")) {
            ...
            val additiveTreeNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("+", -1)
            additiveTreeNode.addChild(applicationPlace.clone())
            additiveTreeNode.addChild(applicationObject.clone())
            additiveTreeNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("-", -1))
            ...
            val additiveSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(additiveTreeNode),
                    code = subst.code, nameEn = subst.nameEn, nameRu = subst.nameRu, priority = subst.priority)
            AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                    applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                    additiveTreeNode,
                    additiveSubstitution,
                    "ComplicatingExtension")
        }
        ...
        if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("MultiplicativeComplicatingExtension") &&
                !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.fastProbabilityCheckOnZero(applicationObject)) //check to avoid divisions on zero. Current verification is only fast appropriate solution, todo: it should be fixed, otherwise it might be possiible to find cheat solutions for some tasks
        {
            ...
            //applicationPlace ~> (applicationPlace * applicationObject) / applicationObject
            val multiplicativeTreeNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("/", -1)
            multiplicativeTreeNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("*", -1))
...
private fun AddApplicationPlaceExtensionSubstitution(
        result: MutableList<SubstitutionApplication>,
        substitutionSelectionData: SubstitutionSelectionData,
        applicationPlace: ExpressionNode, applicationPlaceParent: ExpressionNode, applicationPlaceIndex: Int,
        newNode: ExpressionNode,
        substitution: ExpressionSubstitution,
        substitutionType: String,
        substitutionApplicationError: SubstitutionApplicationError? = null
) {
    applicationPlaceParent.setChildOnPosition(newNode, applicationPlaceIndex)
    result.add(SubstitutionApplication(
            substitution,
            substitutionSelectionData.originalExpression.clone(),
            applicationPlace,
            substitutionSelectionData.expressionToTransform.clone().apply {
                normalizeExpressionToUsualForm(this, substitutionSelectionData.compiledConfiguration)
            }, //because this code performs lots times for lots of substitutions
            newNode,
            substitutionType, substitution.priority ?: 60,
...
private fun tryToGenerateApplicationSubstitutionInstance(substitutionSelectionData: SubstitutionSelectionData, expression: ExpressionNode, substitution: ExpressionSubstitution): SubstitutionInstance {
    var result = substitution.checkLeftCondition(expression, substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator)
    if (!result.isApplicable && substitution.matchJumbledAndNested) {
        ...
        result = substitution.checkLeftCondition(simplifiedExpression, substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator)
    }
    return result
}
...
fun generateSubstitutionsBySelectedNodes(substitutionSelectionData: SubstitutionSelectionData,
                                         ...
                                         withReadyApplicationResult: Boolean = false,
                                         fastestAppropriateVersion: Boolean = false,
                                         withFullExpressionChangingPart: Boolean = false): List<SubstitutionApplication> {
    val result = mutableListOf<SubstitutionApplication>()
        ...
        for (rule in substitutionSelectionData.compiledConfiguration.compiledExpressionSimpleAdditionalTreeTransformationRules) {
            ...
            for (selectedNode in selectedNodes) {
                if (selectedNode.value != rule.right.children.first().value &&
                    ...
                    val applicationResult = rule.checkAndApply(selectedNode, substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator)
    ...
    return fullExpressionChangingPartNormalization(result.distinctBy { it.resultExpression.identifier }.filter {
        it.expressionSubstitution.left.toString() != it.expressionSubstitution.right.toString()
        it.resultExpression.toString() != it.originalExpression.toString() }, withFullExpressionChangingPart)
}

private fun fullExpressionChangingPartNormalization(substitutionApplications: List<SubstitutionApplication>, withFullExpressionChangingPart: Boolean) =
        substitutionApplications.apply {
            if (withFullExpressionChangingPart) {
                forEach {
                    if (it.originalExpressionChangingPart.value != "") {
                        it.originalExpressionChangingPart = ExpressionNode(NodeType.FUNCTION, "").apply {
                            parent = it.originalExpressionChangingPart.parent
                            addChild(it.originalExpressionChangingPart)
                        }
                    }

                    if (it.resultExpressionChangingPart.value != "") {
                        it.resultExpressionChangingPart = ExpressionNode(NodeType.FUNCTION, "").apply {
                            parent = it.resultExpressionChangingPart.parent
                            addChild(it.resultExpressionChangingPart)
                        }
                    }
                }
            }
...
fun generateSubstitutionsBySelectedNodesAndItsPolynomialRoot(substitutionSelectionData: SubstitutionSelectionData,
                                                             polynomialRoot: String,
                                                             withFullExpressionChangingPart: Boolean
...
fun generateSubstitutionsBySelectedNodesAndItsForwardInverseExtension(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.selectedNodes.size == 1) {
        ...
        val applicationObject = forwardInverseExtension.argument

        when (forwardInverseExtension.forwardInverseExtensionType) {
            ForwardInverseExtensionType.ADD_SUBTRACT -> {
                if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("AdditiveComplicatingExtension")) {
                    //applicationPlace ~> applicationPlace + applicationObject - applicationObject
                    val additiveTreeNodePlusMinus = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("+", -1)
                    additiveTreeNodePlusMinus.addChild(applicationPlace.clone())
                    additiveTreeNodePlusMinus.addChild(applicationObject.clone())
                    additiveTreeNodePlusMinus.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("-", -1))
                    additiveTreeNodePlusMinus.children.last().addChild(applicationObject.clone())

                    val plusMinusSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(additiveTreeNodePlusMinus),
                            code = "PlusMinusComplicatingExtension", nameEn = "Plus Minus", nameRu = "Плюс минус", priority = 10)
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            additiveTreeNodePlusMinus,
                            plusMinusSubstitution,
                            plusMinusSubstitution.code)

                    //applicationPlace ~> applicationPlace + applicationObject - applicationObject
                    val additiveTreeNodeMinusPlus = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("+", -1)
                    additiveTreeNodeMinusPlus.addChild(applicationPlace.clone())
                    additiveTreeNodeMinusPlus.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("-", -1))
                    additiveTreeNodeMinusPlus.children.last().addChild(applicationObject.clone())
                    additiveTreeNodeMinusPlus.addChild(applicationObject.clone())

                    val minusPlusSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(additiveTreeNodeMinusPlus),
                            code = "MinusPlusComplicatingExtension", nameEn = "Minus Plus", nameRu = "Минус плюс", priority = 11)
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            additiveTreeNodeMinusPlus,
                            minusPlusSubstitution,
                            minusPlusSubstitution.code)
                }
            }
            ForwardInverseExtensionType.MULTIPLY_DIVIDE -> {
                if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("MultiplicativeComplicatingExtension")) {
                    //TODO: here is sample of error handling
                    //applicationPlace ~> applicationPlace * applicationObject / applicationObject
                    val mulDivNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "/", -1)
                    mulDivNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "*", -1))
                    mulDivNode.children.last().addChild(applicationPlace.clone())
                    mulDivNode.children.last().addChild(applicationObject.clone())
                    mulDivNode.addChild(applicationObject.clone())

                    val mulDivSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(mulDivNode),
                            code = "MulDivComplicatingExtension", nameEn = "Mul Div", nameRu = "Умножить поделить", priority = 10)
                    val error = if (substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.fastProbabilityCheckOnZero(applicationObject)) {
                        SubstitutionApplicationError("object is zero", "деление на 0", arrayOf(), applicationObject)
                    } else null
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            mulDivNode,
                            mulDivSubstitution,
                            mulDivSubstitution.code,
                            error)

                    //TODO: add substitutions  ' * x * ' -> '(* x * a *) / a', ' x / c ' -> '(x * a) / (c * a)', ' c / x ' -> '(c * a) / (x * a)',
                    //TODO: ' * x * / c ' -> '( * x * a * ) / (c * a)', ' * c * / x ' -> '( * c * a * ) / (x * a)', ...
                    //TODO: - normal handling of fractions
                }
            }
            ForwardInverseExtensionType.POW_ROOT -> {
                if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("PowRootComplicatingExtension") &&
                        !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator.fastProbabilityCheckOnZero(applicationObject)) {
                    val applicationPlaceMoreZero = !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator
                            .fastProbabilityCheckOnIncorrectTransformation(applicationPlace, zero, ComparisonType.LEFT_LESS_OR_EQUAL)
                    if (applicationObject.isIntNumber() || applicationPlaceMoreZero) {
                        //TODO: think about error handling
                        //applicationPlace ~> (applicationPlace ^ applicationObject) ^ (1/applicationObject)
                        val powRootNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1)
                        powRootNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1))
                        powRootNode.children.last().addChild(applicationPlace.clone())
                        powRootNode.children.last().addChild(applicationObject.clone())
                        powRootNode.addChild(ExpressionNode(NodeType.FUNCTION, "/"))
                        powRootNode.children.last().addChild(ExpressionNode(NodeType.VARIABLE, "1"))
                        powRootNode.children.last().addChild(applicationObject.clone())

                        val powRootSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(powRootNode),
                                code = "PowRootExtension", nameEn = "Pow Root", nameRu = "Возвести извлечь", priority = 9)
                        AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                                applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                                powRootNode,
                                powRootSubstitution,
                                powRootSubstitution.code)
                    }

                    if (applicationObject.isOddNumber() || applicationPlaceMoreZero) {
                        //applicationPlace ~> (applicationPlace ^ 1/applicationObject) ^ applicationObject   ^(^(applicationPlace;/(1;applicationObject));applicationObject)
                        val rootPowNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1)
                        rootPowNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1))
                        rootPowNode.children.last().addChild(applicationPlace.clone())
                        rootPowNode.children.last().addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "/", -1))
                        rootPowNode.children.last().children.last().addChild(ExpressionNode(NodeType.VARIABLE, "1"))
                        rootPowNode.children.last().children.last().addChild(applicationObject.clone())
                        rootPowNode.addChild(applicationObject.clone())

                        val rootPowSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(rootPowNode),
                                code = "rootPowExtension", nameEn = "Root Pow", nameRu = "Извлечь возвести", priority = 10)
                        AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                                applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                                rootPowNode,
                                rootPowSubstitution,
                                rootPowSubstitution.code)
                    }

                    //TODO: add more substitutions, make it's results simplified
                }
            }
            ForwardInverseExtensionType.EXPONENTIATE_LOGARITHM -> {
                if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("LogExpComplicatingExtension") &&
                        !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator
                                .fastProbabilityCheckOnIncorrectTransformation(applicationObject, zero, ComparisonType.LEFT_LESS_OR_EQUAL) &&
                        !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator
                                .fastProbabilityCheckOnIncorrectTransformation(applicationObject, one, ComparisonType.EQUAL)) { // >0 && !=1
                    val applicationPlaceMoreZero = !substitutionSelectionData.compiledConfiguration.factComparator.expressionComparator
                            .fastProbabilityCheckOnIncorrectTransformation(applicationPlace, zero, ComparisonType.LEFT_LESS_OR_EQUAL)

                    //applicationPlace ~> log ((applicationObject ^ applicationPlace), applicationObject)
                    val expLogNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "log", 2)
                    expLogNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1))
                    expLogNode.children.last().addChild(applicationObject.clone())
                    expLogNode.children.last().addChild(applicationPlace.clone())
                    expLogNode.addChild(applicationObject.clone())

                    val expLogSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(expLogNode),
                            code = "logExpExtension", nameEn = "Log Exp", nameRu = "Log Exp", priority = 9)
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            expLogNode,
                            expLogSubstitution,
                            expLogSubstitution.code)

                    if (applicationPlaceMoreZero) {
                        //applicationPlace ~> applicationObject ^ log(applicationPlace, applicationObject)
                        val logExpNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "^", -1)
                        logExpNode.addChild(substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode( "log", 2))
                        logExpNode.children.last().addChild(applicationPlace.clone())
                        logExpNode.children.last().addChild(applicationObject.clone())
                        logExpNode.addChild(applicationObject.clone())

                        val logExpSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(logExpNode),
                                code = "expLogExtension", nameEn = "Exp Log", nameRu = "Exp Log", priority = 10)
                        AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                                applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                                logExpNode,
                                logExpSubstitution,
                                logExpSubstitution.code)
                    }

                    //TODO: add more substitutions, make it's results simplified
                }
            }
            ForwardInverseExtensionType.LOGIC_ABSORPTION -> {
                if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.containsKey("SetComplicatingExtension")) {
                    //applicationPlace ~> applicationPlace and (applicationPlace or applicationObject)
                    val andOrTreeNode = substitutionSelectionData.compiledConfiguration.createExpressionFunctionNode("and", -1)
                    andOrTreeNode.addChild(applicationPlace.clone())
                    ...
                    val andOrSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(andOrTreeNode),
                            code = "OrAndExtension", nameEn = "Or And", nameRu = "Или И", priority = 10)
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            andOrTreeNode,
                            andOrSubstitution,
                            andOrSubstitution.code)
                    ...
                    val orAndSubstitution = ExpressionSubstitution(addRootNodeToExpression(applicationPlace.clone()), addRootNodeToExpression(orAndTreeNode),
                            code = "OrAndExtension", nameEn = "And Or", nameRu = "И Или", priority = 11)
                    AddApplicationPlaceExtensionSubstitution(result, substitutionSelectionData,
                            applicationPlace, applicationPlaceParent, applicationPlaceIndex,
                            orAndTreeNode,
                            orAndSubstitution,
                            orAndSubstitution.code)
                }
            }
        }
    }
    ...
    }, withFullExpressionChangingPart)
}