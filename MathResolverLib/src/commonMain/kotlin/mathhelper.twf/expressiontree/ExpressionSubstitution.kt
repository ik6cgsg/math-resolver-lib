...
class ExpressionSubstitution(
        ...
        val rightFunctions: Set<String> = right.getContainedFunctions(),
        ...
        val useWhenPostprocessGeneratedExpression: Boolean = false,
    ...
    init {
        ...
        if (!isCopy) {
            right.setLastAppliedSubstitutionForAllNodes(this)
        }
    }

    fun copy(isCopy: Boolean = false, newWeightInTaskAutoGeneration: Double? = null) =
        ExpressionSubstitution(left, right, weight, basedOnTaskContext, code, nameEn, nameRu, comparisonType, leftFunctions, rightFunctions, matchJumbledAndNested,
            priority, changeOnlyOrder, simpleAdditional, isExtending, normalizationType, newWeightInTaskAutoGeneration ?: weightInTaskAutoGeneration, useWhenPostprocessGeneratedExpression,
            isCopy = isCopy)
    ...
    fun setStepIdForRight(stepId: Int) {
        right.setStepIdForAllNodes(stepId)
    }
            ...
            if (right.value.isEmpty() && right.children.size == 1) {
                ...
                if (right.functionStringDefinition?.function?.isNameForRuleDesignations ?: false) {
                    ...
                    return substitutionInstance.getExprFunc(right.value + "_" + right.children.size)
                            ?.cloneWithDeepSubstitutions(functionNameArgsToExpressionMap)
                            ?.normalizeSubTree(sorted = false)
                            ?.apply { right.lastAppliedSubstitution?.let { setLastAppliedSubstitutionForAllNodes(it) } }
                            ?: return null
                } else {
...
data class SubstitutionApplication(
        ...
        var error: SubstitutionApplicationError? = null
) {
...
fun ExpressionNode.addSubstitutionToDiff(compareWith: ExpressionNode, expressionSubstitution: ExpressionSubstitution) {
    if (value != compareWith.value || children.size != compareWith.children.size) {
        lastAppliedSubstitution = expressionSubstitution
        return
    }
    for ((child, compareChild) in children.zip(compareWith.children)) {
        child.addSubstitutionToDiff(compareChild, expressionSubstitution)
    }
}

fun SubstitutionApplication.addSubstitutionToResultExpression() {
    resultExpression.addSubstitutionToDiff(originalExpression, expressionSubstitution)
}

fun ExpressionNode.addStepIdToDiff(compareWith: ExpressionNode, stepId: Int) {
    if (value != compareWith.value || children.size != compareWith.children.size) {
        lastModifiedStepId = stepId
        return
    }
    for ((child, compareChild) in children.zip(compareWith.children)) {
        child.addStepIdToDiff(compareChild, stepId)
    }
}

fun SubstitutionApplication.addStepIdToResultExpression(stepId: Int) {
    resultExpression.addStepIdToDiff(originalExpression, stepId)
}