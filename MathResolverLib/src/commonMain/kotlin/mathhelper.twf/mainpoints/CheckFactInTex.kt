...
fun checkFactsInTex(
    ...
    if (error != null) {
        return TexVerificationResult(returnParsingErrorTex(error, texSolution), "Syntax error (underlined): ${error.description}")
        ...
        if (targetFactPattern.isNotBlank() || otherGoalData != null) {
            ...
            log.addMessage({ "otherGoalData: '''${otherGoalData}'''" }, level = 0)
            log.addMessage({ "hidden goal expression: '''${otherGoalData?.get("hiddenGoalExpressions")}'''" }, level = 0)
            if (otherGoalData != null && otherGoalData["hiddenGoalExpressions"] is List<*>) {
                ...
                log.addMessage({ "hiddenGoalExpressions: '''${hiddenGoalExpressions}'''" }, level = 0)
                ...
                for (hiddenGoalExpression in hiddenGoalExpressions) {
                    ...
                    if (hiddenGoalExpression is String) {
                        ...
                        if (compiledConfiguration.factComparator.expressionComparator.compareAsIs(hiddenGoalExpressionNode, resultExpression.data, withBracketUnification = true)) {
...
private fun returnParsingErrorTex(error: ParserError, tex: String) = returnParsingErrorTex(error.description, error.position, error.endPosition, tex)

private fun returnParsingErrorTex(description: String, start: Int, end: Int, tex: String): String {
    log.addMessage({ "transformations parsing error: '${description}'" }, MessageType.USER, level = 0)
    val endPosition = findClosestPlaceToTargetOnTheSameLevel(tex, start, end, false)
