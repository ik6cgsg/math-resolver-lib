...
fun compiledConfigurationBySettings(
    ...
    if (expressionTransformationRules.isNotEmpty()){
        ...
        compiledConfiguration.expressionTreeAutogeneratedTransformationRuleIdentifiers.putAll(
            expressionTransformationRules.filter { (it.left.nodeType == NodeType.EMPTY || it.right.nodeType == NodeType.EMPTY) && it.code.isNotEmpty() }.map { Pair(it.code, it) }
        )
        compiledConfiguration.compiledExpressionTreeTransformationRules.clear()
        compiledConfiguration.compiledExpressionTreeTransformationRules.addAll(expressionTransformationRules.filter { !(it.left.nodeType == NodeType.EMPTY || it.right.nodeType == NodeType.EMPTY)})
    }
