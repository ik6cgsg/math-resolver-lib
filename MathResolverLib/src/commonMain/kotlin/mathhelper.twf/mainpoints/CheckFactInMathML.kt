...
import mathhelper.twf.standartlibextensions.selectPlacesForColoringByFragment
import mathhelper.twf.visualization.brushMathMl
import mathhelper.twf.visualization.dropPerformedMathMLBrushing
import mathhelper.twf.visualization.setBackgroundColorMathMl
...
//        targetFactPattern: String = "", //Pattern that specify criteria that learner's answer must meet    TODO: refactor this file and switch logic to matching by pattern
    ...
    if (error != null) {
        ...
        val checkingResult = solutionRoot.check(factComparator, false,
                listOf(),
                listOf(),
                log.factConstructorViewer!!.additionalFactsFromItsIdentifiers(additionalFactsIdentifiers) + if (makeFirstSingleTransformationChainFactCorrectWithoutAdditionalFacts == "1") {
                    getListOfExpressionComparisonFactsFromFirstSystem(solutionRoot)
                } else emptyList())
        log.addMessage({
...
private fun returnParsingErrorMathML(error: ParserError, mathML: String) = returnParsingErrorMathML(error.description, error.position, error.endPosition, mathML)

private fun returnParsingErrorMathML(description: String, start: Int, end: Int, mathML: String): String {
        ...
        if (mathML[currentPos] == ')'){
            ...
            if (remainingExpressionStartsWith("</$tag", mathML, currentPos)) {
                ...
                if (tagBracketsStack.isEmpty()){
                    return returnParsingErrorMathML("Not closed tag: '<$tag>'", currentPos - 4, currentPos + 6, mathML)
                }
            ...
            if (tagBracketsStack.isEmpty()){
                return returnParsingErrorMathML("Not closed bracket: '('", currentPos - 4, currentPos + 6, mathML)
            }
...
fun getListOfExpressionComparisonFactsFromFirstSystem (solutionRoot: MainLineAndNode): List<MainChainPart> {
    val firstInputSystem = solutionRoot.factTransformationChains.firstOrNull()?.chain?.firstOrNull() ?: return emptyList()
    if (firstInputSystem is MainLineAndNode) {
        val result = mutableListOf<MainChainPart>()
        for (factTransformationChain in firstInputSystem.factTransformationChains) {
            if (factTransformationChain.chain.size == 1 && factTransformationChain.chain.first() is ExpressionComparison) {
                val fact = factTransformationChain.chain.first() as ExpressionComparison
                result.add(fact)
            }
        }
        return result
