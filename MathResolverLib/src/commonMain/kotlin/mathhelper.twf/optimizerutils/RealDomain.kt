...
// TODO: support using following limits on variable values in domain
data class Segment (
...
class DomainPointGenerator(
        ...
        var fastVersionWithoutConsiderationOfDomain: Boolean = false,
        val numberIntervalType: NumberIntervalType = NumberIntervalType.REAL,
    ...
    val variablesNamesSet = mutableListOf<String>()
    ...
    init {
        ...
        if (numberIntervalType != NumberIntervalType.REAL) {
            fastVersionWithoutConsiderationOfDomain = true
        }

        if (!fastVersionWithoutConsiderationOfDomain) {
            variablesNamesSet.forEach { variablesList.add(VariableInfo(it)) }
            for (expression in _expressions)
                findAllVariables(expression)
            treeNodes.reverse()
            for (attempt in 0 until randomPointCount) {
                variablesList.forEach { it.value = typedRandom(-maxConstant, maxConstant) }
                if (calculatePenaltyForImaginaryValue(variablesList) < epsilon)
                    pointsInDomain.add(clone(variablesList))
            }
            if (pointsInDomain.size < goodEnough) {
    ...
    fun hasVariables() = variablesNamesSet.size > 0
    ...
    fun generateNewPoint (): MutableMap<String, String> {
        ...
        for (variableName in variablesNamesSet){
            result.put(variableName, typedRandom(-border, border).toString())
        }
        return result
    }

    fun generateNewPointInDomain(domainArea: Double = 1e-5, randomPointTries: Int = 10, centreID: Int = -1): MutableMap<String, String> {
        for (iterNumber in 0 until randomPointTries) {
            for (variable in variablesList) {
                variable.value = typedRandom(-2 * maxConstant, 2 * maxConstant)
            }
            if (calculatePenaltyForImaginaryValue(variablesList) < epsilon)
                return variablesListToMap(variablesList)
        }
        if (pointsInDomain.isNotEmpty()) {
            val pointIndex = if (centreID == -1) randomInt(0, pointsInDomain.size) else centreID
            return variablesListToMap(pointsInDomain[pointIndex])
        } else {
            val result = mutableMapOf<String, String>()
            for (variable in variablesList)
                result[variable.name] = typedRandom(-maxConstant, maxConstant).toString()
            return result
        }
    }

    fun typedRandom(from: Double, to: Double) = when (numberIntervalType) {
        NumberIntervalType.SMALL_NATURAL -> randomInt(max(floor(from).toInt(), 1), min(max(ceil(to).toInt(), 1), 5)).toDouble()
        NumberIntervalType.NATURAL -> randomInt(max(floor(from).toInt(), 1), max(ceil(to).toInt(), 1)).toDouble()
        NumberIntervalType.INTEGER -> randomInt(floor(from).toInt(), ceil(to).toInt()).toDouble()
        else -> random(from, to)
    }
    ...
    private fun runGradientDescent(
        ...
        var currentPenalty = calculatePenaltyForImaginaryValue(variableList)
        ...
        for (iterNumber in 0 until iterationCount) {
            ...
            if (ternarySearchIterationCount > 0) {
                ...
                currentPenalty = calculatePenaltyForImaginaryValue(variableList)
            }
            if (currentPenalty < epsilon)
                return true
        }
    ...
    private fun substituteDirectionMultiplier(coef: Double, variableList: List<VariableInfo>, direction: ArrayList<Double>): Double {
        ...
        val penalty = calculatePenaltyForImaginaryValue(variableList)
        for (i in variableList.indices)
            variableList[i].value -= coef * direction[i]
        return penalty
    }
    ...
    private fun calculateDirection(
        ...
        for (variable in variableList) {
            ...
            variable.value += dx
            direction.add(-(calculatePenaltyForImaginaryValue(variableList) - currentPenalty) / dx)
            variable.value -= dx
        }
        return direction
    }
    ...
    private fun runAnnealing(
        ...
        var currentPenalty = calculatePenaltyForImaginaryValue(variablesList)
        ...
        for (iterNumber in 0 until iterationCount) {
            ...
            val varIndex = randomInt(0, newPoint.size)
            newPoint[varIndex] = generateNextValue(newPoint[varIndex], currentPenalty)
            val newPenalty = calculatePenaltyForImaginaryValue(newPoint)
    ...
    private fun calculatePenaltyForImaginaryValue(variableList: List<VariableInfo>): Double {
