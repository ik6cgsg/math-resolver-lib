...
class DoubleBaseOperation {
    companion object {
        ...
        fun implic(listOfArgs: List<Double>) : Double {
            return if (listOfArgs.size != 2) {
                throw IllegalArgumentException("List size is not equals to 2")
            } else {
        ...
        fun setMinus(listOfArgs: List<Double>) : Double {
            return if (listOfArgs.size != 2) {
                throw IllegalArgumentException("List size is not equals to 2")
            } else {
                if (BaseOperationsComputation.additivelyEqual(0.0, listOfArgs[0])) {
                    0.0
                } else if (BaseOperationsComputation.additivelyEqual(0.0, listOfArgs[1])) {
                    1.0
                } else {
                    0.0
                }
