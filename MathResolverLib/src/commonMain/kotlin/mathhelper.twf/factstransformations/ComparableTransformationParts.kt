...
import mathhelper.twf.config.CheckingKeyWords.Companion.factChainVerified
import mathhelper.twf.config.CheckingKeyWords.Companion.inFact
import mathhelper.twf.config.CheckingKeyWords.Companion.ruleAddedInContext
import mathhelper.twf.config.CheckingKeyWords.Companion.verificationFailed
import mathhelper.twf.expressiontree.*
import mathhelper.twf.logs.MessageType
import mathhelper.twf.logs.log
import mathhelper.twf.standartlibextensions.SplittingString
import mathhelper.twf.standartlibextensions.splitBySubstringOnTopLevel
import mathhelper.twf.standartlibextensions.splitStringByBracketsOnTopLevel
import mathhelper.twf.visualization.ColoringTask
...
class RulePointer(
    ...
    override fun check(factComparator: FactComparator, onExpressionLevel: Boolean,
                       ...
                       skipTrivialCheck: Boolean): ComparisonResult {
    ...
    override fun isFactorizationForVariables(minNumberOfMultipliers: Int, targetVariables: Set<String>, targetExpression: ExpressionNode, factComparator: FactComparator): GeneralError? = null
    override fun hasNoFractions(maxNumberOfDivisions: Int, targetExpression: ExpressionNode, factComparator: FactComparator): GeneralError? = null
    override fun isSolutionWithoutFunctions(forbidden: List<Pair<String,Int>>, targetExpression: ExpressionNode, factComparator: FactComparator): GeneralError? = null
...
class Rule(
    ...
    override fun check(factComparator: FactComparator, onExpressionLevel: Boolean,
        ...
        val comparisonResult = root.check(factComparator, false,
                factsTransformations, expressionTransformations, additionalFacts, skipTrivialCheck)
            ...
            if (root.expressionTransformationChains.isNotEmpty()) {
                ...
                if (!factComparator.compareAsIs(left, right)) {
                    factSubstitution = FactSubstitution(left, right,
                            basedOnTaskContext = comparisonResult.additionalFactUsed,
                            name = name,
                            factComparator = factComparator)//todo: add direction (and ability for user to declare it)
        ...
        while (currentRightIndex < chain.size) {
            ...
            if (chain[currentRightIndex].type() == ComparableTransformationPartType.RULE ||
                ...
                if (chain[currentRightIndex].type() == ComparableTransformationPartType.RULE) {
                    ...
                    val checkingResult = rule.check(factComparator, false, factsTransformations, expressionTransformations, additionalFacts, skipTrivialCheck)
            ...
            if (chain[currentLeftIndex].type() == chain[currentRightIndex].type()) {
                ...
                if (chain[currentLeftIndex].type() == ComparableTransformationPartType.EXPRESSION_COMPARISON) {
                    ...
                    if (leftExpressionComparison.comparisonType == rightExpressionComparison.comparisonType) {
                        ...
                        val subtractionResult = factComparator.expressionComparator.compareWithoutSubstitutions(leftSubtraction, rightSubtraction, ComparisonType.EQUAL)
                        ...
                        if (!transformationVerified) {
                            when (leftExpressionComparison.comparisonType) {
                                ComparisonType.EQUAL -> {
                                    val ll = factComparator.expressionComparator.compareWithTreeTransformationRules(leftExpressionComparison.leftExpression.data,
                                    ...
                                    val rr = factComparator.expressionComparator.compareWithTreeTransformationRules(leftExpressionComparison.rightExpression.data,
                                    ...
                                    if (ll && rr) {
                                        ...
                                        val lr = factComparator.expressionComparator.compareWithTreeTransformationRules(leftExpressionComparison.leftExpression.data,
                                                rightExpressionComparison.rightExpression.data, actualExpressionTransformations,
                                                maxDistBetweenDiffSteps = factComparator.compiledConfiguration.comparisonSettings.maxDistBetweenDiffSteps)
                                        log.add(lr, { "left.left are equal to right.right: '" }, { "'" }, level = currentLogLevel, messageType = MessageType.USER)
                                        val rl = factComparator.expressionComparator.compareWithTreeTransformationRules(leftExpressionComparison.rightExpression.data,
                                                rightExpressionComparison.leftExpression.data, actualExpressionTransformations,
                                                maxDistBetweenDiffSteps = factComparator.compiledConfiguration.comparisonSettings.maxDistBetweenDiffSteps)
                                    ...
                                    if (!transformationVerified) {
                                        ...
                                        val divisionResult = factComparator.expressionComparator.compareWithoutSubstitutions(leftDivision, rightDivision, ComparisonType.EQUAL,
                    ...
                    if (leftFacts != null && rightFacts != null) {
                        ...
                        if (!hasDifferentComparisonType) {
                            ...
                            for (i in 0..leftFacts.lastIndex) {
                                ...
                                val result = mainChain.check(factComparator, true, actualFactsTransformations,
                                        actualExpressionTransformations + expressionTransformationRulesFromLeftFact, listOf(), skipTrivialCheck)
                                if (!result.isCorrect) {
...
//        }
    ...
    override fun check(factComparator: FactComparator, onExpressionLevel: Boolean,
        ...
        for (factChain in factTransformationChains) {
            val checkingResult = factChain.chain.first().check(factComparator, false, factsTransformations, expressionTransformations, additionalFacts, skipTrivialCheck)
        ...
        for (rule in rules) {
            val checkingResult = rule.check(factComparator, false,
        ...
        for (expressionChain in expressionTransformationChains) {
            val checkingResult = expressionChain.check(factComparator, false,
                    factsTransformations + nodeFactsTransformations,
                    expressionTransformations + nodeExpressionTransformations,
                    additionalFacts + inFacts,
                    skipTrivialCheck)
        ...
        for (factChain in factTransformationChains) {
            val checkingResult = factChain.check(factComparator, false,
                    factsTransformations + nodeFactsTransformations,
                    expressionTransformations + nodeExpressionTransformations,
                    additionalFacts + inFacts,
                    skipTrivialCheck)
    ...
    override fun isFactorizationForVariables(minNumberOfMultipliers: Int, targetVariables: Set<String>, targetExpression: ExpressionNode, factComparator: FactComparator): GeneralError? {
        if (outFacts.isNotEmpty()) {
            return outFacts.last().isFactorizationForVariables(minNumberOfMultipliers, targetVariables, targetExpression, factComparator)
        }
...
fun MainLineAndNode.checkTransformationChain(
        ...
        additionalFacts: List<MainChainPart>,
        skipTrivialCheck: Boolean = false): ComparisonResult {
    ...
    return this.check(factComparator,
            onExpressionLevel,
